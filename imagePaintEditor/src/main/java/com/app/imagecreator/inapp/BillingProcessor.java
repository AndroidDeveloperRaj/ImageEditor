...
public class BillingProcessor extends BillingBase {
    ...
    private void bindPlayServices() {
        try {
            ...
            Utility.log(LOG_TAG, e.toString());
        }
    }
    ...
    private TransactionDetails getPurchaseTransactionDetails(String productId, BillingCache cache) {
        ...
        if (details != null && !TextUtils.isEmpty(details.responseData)) {
            try {
                ...
                Utility.log(LOG_TAG, "Failed to load saved purchase details for " + productId);
            }
        }
        return null;
    }
        ...
        try {
            ...
            if (transactionDetails != null && !TextUtils.isEmpty(transactionDetails.purchaseToken)) {
                ...
                if (response == Constants.BILLING_RESPONSE_RESULT_OK) {
                    ...
                    Utility.log(LOG_TAG, "Successfully consumed " + productId + " purchase.");
                    return true;
                } else {
                    if (eventHandler != null)
                        eventHandler.onBillingError(response, null);
                    Utility.log(LOG_TAG, String.format("Failed to consume %s: error %d", productId, response));
                }
            }
        } catch (Exception e) {
            Utility.log(LOG_TAG, e.toString());
        ...
        if (billingService != null && productIdList != null && productIdList.size() > 0) {
            try {
                ...
                if (response == Constants.BILLING_RESPONSE_RESULT_OK) {
                    ...
                    if (eventHandler != null)
                        eventHandler.onBillingError(response, null);
                    Utility.log(LOG_TAG, String.format("Failed to retrieve info for %d products, %d", productIdList.size(), response));
                }
            } catch (Exception e) {
                Utility.log(LOG_TAG, String.format("Failed to call getSkuDetails %s", e.toString()));
            }
        }
        return null;
    }
    ...
    public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode != PURCHASE_FLOW_REQUEST_CODE)
            return false;
        if (data == null) {
            Utility.log(LOG_TAG, "handleActivityResult: data is null!");
            return false;
        }
        int responseCode = data.getIntExtra(Constants.RESPONSE_CODE, Constants.BILLING_RESPONSE_RESULT_OK);
        Utility.log(LOG_TAG, String.format("resultCode = %d, responseCode = %d", resultCode, responseCode));
            ...
            try {
                ...
                if (purchasePayload.equals(developerPayload)) {
                    if (verifyPurchaseSignature(productId, purchaseData, dataSignature)) {
                        ...
                        Utility.log(LOG_TAG, "Public key signature doesn't match!");
                        if (eventHandler != null)
                            eventHandler.onBillingError(Constants.BILLING_ERROR_INVALID_SIGNATURE, null);
                    }
                } else {
                    Utility.log(LOG_TAG, String.format("Payload mismatch: %s != %s", purchasePayload, developerPayload));
                    if (eventHandler != null)
                        eventHandler.onBillingError(Constants.BILLING_ERROR_INVALID_SIGNATURE, null);
                }
            } catch (Exception e) {
